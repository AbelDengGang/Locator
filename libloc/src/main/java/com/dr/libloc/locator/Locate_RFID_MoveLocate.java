...
public class Locate_RFID_MoveLocate extends Locate_Impl {
    ...
    public Locate_RFID_MoveLocate(Locator locator) {
    ...
    public void calPosotion(){
        if(isFirst){
            ...
            for (int i=(rfInfos.size() - 15); i < rfInfos.size(); i++){
                float itemx = rfInfos.get(i).mapItem.getPositionPost().getCX();
                float itemy = rfInfos.get(i).mapItem.getPositionPost().getCY();
